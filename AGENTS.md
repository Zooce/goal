...
**Shared list path.** When two flows both iterate a dir, load each file, and print, prefer one generic `Dir.listItems(ctx, Item, opts)` (comptime `Item` with `init` / `deinit` / `printListLine` / `print`) over separate list implementations. Use `Dir.isEmpty` for "no file entries" when a domain dir only holds that domain's files.
