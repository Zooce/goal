...
pub fn run(alloc_: std.mem.Allocator, stdout_: *std.io.Writer, ids_: std.ArrayList([]const u8)) !void {
    ...
    const choices = if (ids_.items.len > 0) ids_.items else try cli.getGoalChoices(alloc_, stdout_, dirs);
    ...
    try dirs.listSome(alloc_, stdout_, choices);
    ...
    for (choices) |choice| {
        ...
        try dirs.base_dir.deleteFile(choice);
    }
